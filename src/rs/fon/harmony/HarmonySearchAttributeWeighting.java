...
public class HarmonySearchAttributeWeighting extends OperatorChain {
	...
	// Constructor
	public HarmonySearchAttributeWeighting(OperatorDescription description)
					...
					if (RandomGenerator.getRandomGenerator(this).nextDouble() < pitchAdjustmentRate) {
						for (int j = 0; j < array.length; j++) {
							if (RandomGenerator.getRandomGenerator(this)
								...
								listOfPitches.get(i1)[j] = p.perturb(j,
										listOfPitches.get(i1)[j]
												* (1 + improvisation));
								if (listOfPitches.get(i1)[j] > 1)
									listOfPitches.get(i1)[j] = 1;
								if (listOfPitches.get(i1)[j] < 0) {
									listOfPitches.get(i1)[j] = 0;
								}
							} else {
								listOfPitches.get(i1)[j] = p.perturb(j,
										listOfPitches.get(i1)[j]
