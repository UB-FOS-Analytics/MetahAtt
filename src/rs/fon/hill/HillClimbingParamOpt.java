...
public class HillClimbingParamOpt extends ParameterOptimizationOperator {
	...
	public HillClimbingParamOpt(OperatorDescription description) {
		super(description);
	}
		...
		case 1: // Default iterated
			for (int i = 0; i < no_of_iterations; i++) {
				...
				for (int j = 0; j < no_of_iterations2; j++) {
					...
					int e = RandomGenerator.getRandomGenerator(this).nextInt(
							v.length);
					v[e] = p.perturb(e, v[e]);
			...
			for (int i = 0; i < no_of_iterations; i++) {
				...
				if (solution.getMainCriterion().getFitness() > best
					...
					double rand = RandomGenerator.getRandomGenerator(this)
							.nextDouble();
					double exp = (1. / (1. + Math.exp((best.getMainCriterion()
